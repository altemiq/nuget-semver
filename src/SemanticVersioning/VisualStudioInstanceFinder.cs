...
namespace Mondo.SemanticVersioning
{
    ...
    internal class VisualStudioInstanceFinder
    {
        ...
        private Microsoft.Build.Locator.VisualStudioInstance GetVisualStudioInstance(string globalJson, NuGet.Versioning.SemanticVersion? requested, bool allowPrerelease, RollForwardPolicy policy)
        {
            ...
            {
                throw new InvalidOperationException(FormattableString.Invariant($"A compatible installed dotnet SDK for global.json version: [{requested}] from [{globalJson}] was not found{Environment.NewLine}Please install the [{requested}] SDK or update [{globalJson}] with an installed dotnet SDK:{Environment.NewLine}  {string.Join(Environment.NewLine + "  ", this.instances.Select(instance => $"{instance.Value.Version} [{instance.Value.MSBuildPath}]"))}"));
            }
